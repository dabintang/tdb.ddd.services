...
using tdb.ddd.relationships.application.contracts;
using tdb.ddd.webapi;

namespace tdb.ddd.relationships.webapi.Controllers
{
    /// <summary>
    /// 控制器基类
    /// </summary>
    ...
    [Authorize]
    [TdbAuthClientIP()]
    [Route("tdb.ddd.relationships/v{api-version:apiVersion}/[controller]/[action]")]
    public class BaseController : ControllerBase
    {
        ...
        protected virtual TdbOperateReq<T> CreateTdbOperateReq<T>(T param)
        {
            var reqOpe = new TdbOperateReq<T>(param, this.CurUser?.ID ?? 0, this.CurUser?.Name ?? "")
            {
                ...
                AuthenticationInfo = HttpContext.GetAuthenticationHeaderValue()
            };
